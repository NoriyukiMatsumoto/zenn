...
```bash
puppeteer-aws-sample$ sam build
```
...
```bash
puppeteer-aws-sample$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
puppeteer-aws-sample$ sam local start-api
puppeteer-aws-sample$ curl http://localhost:3000/
```
...
```bash
puppeteer-aws-sample$ sam logs -n HelloWorldFunction --stack-name puppeteer-aws-sample --tail
```
...
```bash
puppeteer-aws-sample$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name puppeteer-aws-sample
```
